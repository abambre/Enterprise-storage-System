...
void usePersistentFile(FILE **fp)
{
  ...
  while (1) 
  {
    ...
    {
      printf("inside the geline break\n");
       break;
    }
    ...
    if(isfile ==0)
    {
        ...
        printf("filelen <%ld>\n",filelen);
        //char buffer[PERSIST_READ_BUFF_SIZE];
        //memset(buffer,'\0',PERSIST_READ_BUFF_SIZE);
        ...
        printf("return status from rmfs_create <%d>\n",status);
        if(status !=0)
          break;
        if(strcmp(buffer,"EMPTY\n")!=0)
        {
          printf("return status from rmfs_write <%d>\n",status);
    ...
    else
    {
      ...
      printf("return status from rmfs_mkdir <%d>\n",status);
      if(status !=0)
          break;
    }
